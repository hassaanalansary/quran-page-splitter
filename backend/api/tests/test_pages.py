...
class ReviewDataTests(TestCase):
    ...
    def test_empty(self):
        self.assertEqual(editing.review_data(self.mushaf.id), {"start": {"sura": 1, "aya": 1}, "pages": []})

    def test_seed_from_run_and_ordered(self):
        _seed_pages(self.mushaf, 2, {"start_sura": 2, "start_aya": 5})
        data = editing.review_data(self.mushaf.id)
    ...
    def test_pages_carry_reviewed_flag(self):
        ...
        self.assertFalse(editing.review_data(self.mushaf.id)["pages"][0]["reviewed"])
        Page.objects.filter(mushaf=self.mushaf, page_number=1).update(reviewed=True)
        self.assertTrue(editing.review_data(self.mushaf.id)["pages"][0]["reviewed"])

    def test_pages_carry_run_start(self):
        _seed_pages(self.mushaf, 2, {"start_sura": 2, "start_aya": 5})
        pages = editing.review_data(self.mushaf.id)["pages"]
...
class BulkSaveTests(TestCase):
    ...
    def test_writes_lines_without_flipping_reviewed(self):
        editing.bulk_save(mushaf_id=self.mushaf.id, pages=[self._one_line_payload(1)], reviewed_pages=[])
    ...
    def test_reviewed_flags_only_listed_pages(self):
        editing.bulk_save(mushaf_id=self.mushaf.id, pages=[], reviewed_pages=[1, 3])
    ...
    def test_single_review_event(self):
        editing.bulk_save(mushaf_id=self.mushaf.id, pages=[], reviewed_pages=[2])
    ...
    def test_range_review_event(self):
        editing.bulk_save(mushaf_id=self.mushaf.id, pages=[], reviewed_pages=[3, 1, 2])
        event = ActivityEvent.objects.get(mushaf=self.mushaf, type=ActivityTypeChoices.REVIEW_SAVED)
        self.assertEqual(event.payload, {"page_start": 1, "page_end": 3, "count": 3})

    def test_no_event_when_already_reviewed(self):
        editing.bulk_save(mushaf_id=self.mushaf.id, pages=[], reviewed_pages=[1])
        editing.bulk_save(mushaf_id=self.mushaf.id, pages=[], reviewed_pages=[1])
        self.assertEqual(
    ...
    def test_returns_review_data(self):
        out = editing.bulk_save(mushaf_id=self.mushaf.id, pages=[], reviewed_pages=[])
    ...
    def test_creates_manual_page(self):
        ...
        editing.bulk_save(mushaf_id=self.mushaf.id, pages=[self._one_line_payload(5)], reviewed_pages=[5])
